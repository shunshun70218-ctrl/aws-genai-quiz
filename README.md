...
- **自訂常錯題**：在開始畫面輸入題號（可多個，以逗號或空格分隔），就只練習指定的那幾題，方便針對常錯題加強。
- **「中文」按鈕一鍵切換中英雙語**：題幹與選項顯示英文＋繁體中文對照（AWS 服務名與技術術語保留英文）；作答中或看錯題回顧時皆可切換。
- **可選「依文件順序顯示」**：勾選後題目依題號排序、不隨機打亂（隨機與自訂題號模式皆適用）。
